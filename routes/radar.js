...
/**
  * function to repeatedly request and save rainRadar data
  */
function intervalRainRadar(db) {
  let refresh = config.refresh_rate;
  let prod;
  // if refresh is smaller than 1h
  if(refresh < 3600000) {
    prod = "RY";
  } else {
    prod = "RW";
  ...
  .then(function(fileList) {
    ...
    // check the database for that latest radar product
    checkDatabase(prod, latestDateStamp, db)
    .catch(console.error)
    .then(function(response) {
      // when the latest product is not yet in the database
      if(!response) {
        ...
        promiseToFetchRadarData(prod)
          ...
          .then(function(radarPolygonsJSON) {
            try {
              // post it to DB
              promiseToPostItems([radarPolygonsJSON], db)
                .catch(console.error)
                .then( function() {
                  console.log("new " + prod + " product saved.");
                }
              );
            } catch (e) {
              console.dir(e);
      ...
      // if the data is found in the database
        ...
        console.log("radar data found, no new fetch needed");
      }
    });
  });
}
