...
/**
* This function calls '/db/add' with AJAX, to save a given item in the database.
* The logic is wrapped in a promise to make it possible to await it (see processUnwetterFromDWD for an example of await)
* @author Paula Scharf, matr.: 450334
* @param {Object} item - the item to be posted
* @param {String} typeOfItem - for documentation
*/
function promiseToPostItem(item, typeOfItem) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // if the request is done successfully, ...
    .done(function (response) {
      // ... give a notice on the console that the AJAX request for inserting an item has succeeded
      console.log("AJAX request (inserting one " + typeOfItem + ") is done successfully.");
      resolve();
    })

    // if the request has failed, ...
    .fail(function (xhr, status, error) {
      // ... give a notice that the AJAX request for inserting an item has failed and show the error on the console
      console.log("AJAX request (inserting one " + typeOfItem + ") has failed.", error);

      // send JSNLog message to the own server-side to tell that this ajax-request has failed because of a timeout
      if (error === "timeout") {
        //JL("ajaxInsertingOneItemTimeout").fatalException("ajax: '/add' timeout");
      }

      reject("AJAX request (inserting one " + typeOfItem + ") has failed.");
    });
  });
}
...
/**
* This function calls '/db/addMany' with AJAX, to save the given items in the database.
* @author Paula Scharf
* @param arrayOfItems - array which contains the items
* @param {String} typeOfItems - for documentation
*/
function promiseToPostMany(arrayOfItems, typeOfItems) {
