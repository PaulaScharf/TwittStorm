...
list_length <- nr_of_classes + meta_offset
...
# count classes in @data$DN, all_pol is 1,2 + 1,2,3,4 = 6
all_pol[[3]] <- vector("list", table(pol@data[[1]]))
all_pol[[4]] <- vector("list", table(pol@data[[2]]))
all_pol[[5]] <- vector("list", table(pol@data[[3]]))
all_pol[[6]] <- vector("list", table(pol@data[[4]]))
length(table(pol@data))

# all_pol 3,4,5,6 are sub-lists
